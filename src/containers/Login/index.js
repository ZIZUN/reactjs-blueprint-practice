    ...
    return(
      <div style={{backgroundColor: '#ffffff'}}  >
        ...
        <form className="aform" style={{ backgroundColor: '#f8f361'}}  onSubmit={this.onSubmit.bind(this)}>
          ...
          <div style={{width: '420px',margin: '0 auto',marginTop: '20px',padding: '50px'}}>
            <div style={{height: '70px'}}>
              <input className="pt-input" style={{width: "320px",height: '50px'}}  placeholder="아이디" type="text" dir="auto" onChange={this.onChange.bind(this,'email')}/>
              {!!this.showError('email') ? <p className="error-message">{this.getError('email')} </p> : null}
            </div>
            <div style={{height: '70px'}}>
              <input className="pt-input" style={{width: "320px",height: '50px'}} placeholder="비밀번호" type="password" dir="auto" onChange={this.onChange.bind(this,'password')}/>
              {!!this.showError('password') ? <p className="error-message">{this.getError('password')} </p> : null}
            </div>
            <button class="pt-button pt-intent-primary" style={{ width: 320,height: '50px'}} type="submit" >로그인</button>
