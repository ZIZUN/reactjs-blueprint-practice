        ...
        <form onSubmit={this.onSubmit.bind(this)}>
          <div style={{width: '500px',margin: '0 auto',marginTop: '200px',padding: '50px'}}>
            <div style={{height: '70px'}}>
              <input className="pt-input" style={{width: "400px",height: '50px'}} value={profile && profile.email} placeholder="아이디" type="email" dir="auto" onChange={this.onChange.bind(this,'email')}/>
              {!!this.showError('email') ? <p className="error-message">{this.getError('email')} </p> : null}
            </div>
            <div style={{height: '70px'}}>
              <input className="pt-input" style={{width: "400px",height: '50px'}} value={profile && profile.password} placeholder="비밀번호" type="password" dir="auto" onChange={this.onChange.bind(this,'password')}/>
              {!!this.showError('password') ? <p className="error-message">{this.getError('password')} </p> : null}
            </div>
            <div style={{height: '70px'}}>
              <input className="pt-input" style={{width: "400px",height: '50px'}} value={profile && profile.confirm_password} placeholder="비밀번호 확인" type="password" dir="auto" onChange={this.onChange.bind(this,'confirm_password')}/>
              {!!this.showError('confirm_password') ? <p className="error-message">{this.getError('confirm_password')} </p> : null}
            </div>
            <button class="pt-button pt-intent-primary" style={{width: 400,height: '50px'}} type="submit">수정하기</button>
          ...
          <div style={{backgroundColor: '#245541', height: 500}}/>
        </form>
      </div>
